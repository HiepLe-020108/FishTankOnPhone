...
namespace Mkey
{
    public class SimpleTween : MonoBehaviour
    {
        ...
        {
            get { return maxCount; }
            set { maxCount = (value < 400) ? 400 : value; }
        }
        ...
        public class SimpleTweenObject
        {
            ...
            float maxValue;
            STT stt;
            private CancelCondition cancelCondition = CancelCondition.AsIs;
            ...
            public void Update(float deltaTime)
            {
                ...
                switch (stt)
                {
                    ...
                    case STT.Vector3Tween:
                        if (!isCanceled) currPosition = (timeLeft) ? endPosition : startPosition + new Vector3(EaseFunc(currTime, dPosition.x, tweenTime), EaseFunc(currTime, dPosition.y, tweenTime), EaseFunc(currTime, dPosition.z, tweenTime));
                        if (UpdateCallBackV3 != null) UpdateCallBackV3(currPosition);
                        break;
                ...
                {
                   if( onComplete) if (completeCallBack != null) completeCallBack();
                }
            }
